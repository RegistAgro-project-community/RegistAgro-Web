              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-background border-b border-border-color uppercase">
                    ...
                    <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-text-secondary">Produto</th>
                    <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-text-secondary">Quantidade</th>
                ...
                <tbody className="divide-y divide-border-color">
                  {isLoading ? (
                    [...Array(5)].map((_, i) => (
                      <tr key={i}>
                        <td className="px-6 py-5"><Skeleton variant="text"    height={20} width={130} animation="wave" sx={{ bgcolor: '#f0f0f0' }} /></td>
                        <td className="px-6 py-5"><Skeleton variant="text"    height={20} width={150} animation="wave" sx={{ bgcolor: '#f0f0f0' }} /></td>
                        <td className="px-6 py-5"><Skeleton variant="text"    height={20} width={60}  animation="wave" sx={{ bgcolor: '#f0f0f0' }} /></td>
                        <td className="px-6 py-5"><Skeleton variant="text"    height={20} width={90}  animation="wave" sx={{ bgcolor: '#f0f0f0' }} /></td>
                        <td className="px-6 py-5 flex justify-center">
                          <Skeleton variant="rounded" height={22} width={75} animation="wave" sx={{ bgcolor: '#f0f0f0', borderRadius: '999px' }} />
                        </td>
                        <td className="px-6 py-5">
                          <div className="flex items-center justify-end gap-2">
                            <Skeleton variant="rounded" height={28} width={65}  animation="wave" sx={{ bgcolor: '#f0f0f0', borderRadius: '8px' }} />
                            <Skeleton variant="rounded" height={28} width={65} animation="wave" sx={{ bgcolor: '#f0f0f0', borderRadius: '8px' }} />
                          </div>
                        </td>
                      </tr>
                    ...
                    correntItems.map((item, i) => (
                      <tr key={i} className="hover:bg-gray-50 transition-colors group">
                        <td className="px-6 py-5">
                          <span className="text-sm font-semibold text-text-main">{item.consumer.name}</span>
                        </td>
                        <td className="px-6 py-5">
                          <p className="text-base font-medium text-text-secondary leading-relaxed capitalize">{item.product.name}</p>
                        </td>
                        <td className="px-6 py-5">
                          <p className="text-sm text-text-secondary font-medium">{item.qtd}{item.unit}</p>
                        </td>
                        <td className="px-6 py-5">
                        ...
                        <td className="px-6 py-5 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0 rounded-full text-xs font-medium ${
                            item.status === "pendent"   ? "bg-yellow-100 text-yellow-800 border border-yellow-200" :
                            item.status === "confirmed" ? "bg-green-100 text-green-800 border border-green-200"   : ""
                          }`}>
                            {item.status}
                          </span>
                        </td>
